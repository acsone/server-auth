...
class DeviceController(Home):
    ...
    def device_login(self, redirect=None, **kw):
        ensure_db()
        ...
        if not request.params["device_code"]:
            return werkzeug.utils.redirect("/web/login?auth_device_error=2")
        user = (
        ...
        if not user:
            ...
            try:
                request.params["password"] = request.params["device_code"]
                uid = request.session.authenticate(
                    request.session.db, user.login, request.params["password"]
                )
